...
# Bestehende Funktionen in helper_functions.py
...
def place_objects_in_image_ft(background_files, objects, image_height, image_width, num_objects, num_objects_std, class_percentages, rotation_range, scale_range, allow_overlap, use_backgrounds):
    ...
    # Berechne die Anzahl der Objekte für jede Klasse mit einer gewissen Zufälligkeit
    class_counts = {}
    for class_id, percentage in class_percentages.items():
        count = max(0, int(target_count * (percentage / total_percentage)))
        class_counts[class_id] = count
    remaining_count = target_count - sum(class_counts.values())
    
    while remaining_count > 0:
        class_id = random.choice(list(class_percentages.keys()))
...
# Hinzufügen der neuen Funktionen aus generate_txt_for_img.py
